...
document.addEventListener('DOMContentLoaded', () => {
    ...
    examenForm.addEventListener('submit', async (event) => {
        ...
        const duracionHoras = parseInt(document.getElementById('duracionHoras').value, 10);
        ...
        // Validación para verificar que haya al menos una pregunta
        if (preguntas.length === 0) {
            alert('Debe agregar al menos una pregunta para poder guardar el examen.');
            return; // Detener el proceso si no hay preguntas
        }
    
        // Validación de duración mínima (no 0 horas y 0 minutos, al menos 10 minutos)
        if (duracionHoras === 0 && duracionMinutos < 10) {
            alert('La duración mínima del examen debe ser de al menos 10 minutos.');
            return; // Detener el proceso si la duración es menor a 10 minutos
        }
