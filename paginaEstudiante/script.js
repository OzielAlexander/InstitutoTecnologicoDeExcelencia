...
document.getElementById('buscarExamen').addEventListener('click', async () => {
    ...
    document.querySelectorAll('.ingresar-btn').forEach(boton => {
        boton.addEventListener('click', async () => {
            ...
            // Verificar la fecha de cierre antes de ingresar
            const permitido = await verificarFechaDeCierre(examenId);
            if (permitido) {
                ...
                alert("El examen ya ha sido cerrado y no se puede ingresar.");
            }
        });
    });
});

// Función para verificar la fecha de cierre
const verificarFechaDeCierre = async (examenId) => {
    try {
        ...
        if (examenSnap.exists()) {
            ...
            const fechaCierre = new Date(examenData.cierre); // Convertir la fecha de cierre a un objeto Date
            const fechaActual = new Date(); // Obtener la fecha y hora actual

            // Comparar la fecha actual con la fecha de cierre
            return fechaActual <= fechaCierre; // Si la fecha actual es menor o igual, permitir el acceso
        } else {
            alert("No se encontró el examen.");
            return false;
        }
    } catch (error) {
        console.error("Error al verificar la fecha de cierre:", error);
        alert("Ocurrió un error al verificar la fecha de cierre. Inténtalo de nuevo.");
        return false;
    }
};
